...
# OCR module
sys.path.append(os.path.abspath(os.path.join(base_dir, '..', 'ocr')))
try:
    from extractor import extract_ticket_fields
    OCR_AVAILABLE = True
except ImportError:
    OCR_AVAILABLE = False
...
if "ocr_log" not in st.session_state:
    st.session_state.ocr_log = []  # Track OCR accuracy over time
...
def render_navbar(active_page):
    ...
    with c2:
        if st.button(" Predictor", type="primary" if active_page=="predictor" else "secondary", use_container_width=True):
            ...
            st.rerun()
    with c3:
        if st.button("📷 Scan Ticket", type="primary" if active_page=="scan" else "secondary", use_container_width=True):
            go_scan()
            st.rerun()
    with c4:
...
# Confidence threshold for the UI
CONFIDENCE_THRESHOLD_UI = 0.75

# --- PAGE: SCAN TICKET (OCR) ---
def render_scan_ticket():
    render_navbar("scan")
    st.markdown("## 📷 Scan Your Boarding Pass / E-Ticket")
    ...
    if not encoders:
        st.error("Encoders not found. Please train the models first.")
        return

    airlines = list(encoders['airline'].classes_)
    origins = list(encoders['origin'].classes_)
    ...
    with col_upload:
        with st.container(border=True):
            st.subheader("📄 Upload Ticket")
            uploaded_file = st.file_uploader(
                "Upload a photo or PDF of your boarding pass / e-ticket",
            ...
            if uploaded_file is not None:
                ...
                if uploaded_file.type == "application/pdf":
                    st.info("📝 PDF detected — the first page will be scanned.")
                    # Write PDF to a temp file for pdf2image
                    tmp_dir = os.path.join(base_dir, '..', '.tmp_ocr')
                    ...
                    st.image(pil_img, caption="Uploaded Ticket Preview", use_container_width=True)
                    source = pil_img

                # --- Run OCR ---
                if st.button("🔍 Extract Fields", type="primary", use_container_width=True):
                    with st.spinner("Running OCR engine — preprocessing & extracting fields..."):
                        try:
                            result = extract_ticket_fields(source)
    ...
    with col_result:
        with st.container(border=True):
            ...
            if "ocr_result" not in st.session_state:
                st.info("Upload an image or PDF and click **Extract Fields** to begin.")
                return

            ocr = st.session_state.ocr_result

            # Overall confidence badge
            overall = float(ocr['overall_ocr_confidence']['value'])
            conf_pct = f"{overall * 100:.0f}%"
            if overall >= CONFIDENCE_THRESHOLD_UI:
                st.success(f"🎯 Overall OCR Confidence: **{conf_pct}**")
            else:
                st.warning(f"⚠️ Overall OCR Confidence: **{conf_pct}** — some fields may need correction.")

            st.markdown("---")

            # --- Editable fields ---
            def _field_input(label, ocr_field, options=None, key_suffix=""):
                """Render a text_input or selectbox for an OCR field, with warning if low confidence."""
                field = ocr.get(ocr_field, {"value": "", "confidence": 0.0, "low_confidence": True})
                if field.get("low_confidence"):
                    st.warning(f"⚠️ **{label}**: OCR unsure — please verify")
                if options:
                    # Try to match extracted value to an option
                    default_idx = 0
                    val = field.get("value", "")
                    if val in options:
                        default_idx = options.index(val)
                    return st.selectbox(label, options, index=default_idx, key=f"ocr_{ocr_field}{key_suffix}")
                else:
                    return st.text_input(label, value=field.get("value", ""), key=f"ocr_{ocr_field}{key_suffix}")

            fc1, fc2 = st.columns(2)
            with fc1:
                scanned_flight = _field_input("Flight Number", "flight_number")
                scanned_airline = _field_input("Airline", "airline", options=airlines)
                scanned_origin = _field_input("Origin (IATA)", "origin", options=origins)
            with fc2:
                scanned_destination = _field_input("Destination (IATA)", "destination", options=destinations)
                scanned_date = _field_input("Date", "date")
                scanned_time = _field_input("Departure Time", "time")

            st.markdown("---")

            # --- Expandable raw text ---
            with st.expander("🔎 View raw OCR text"):
                st.code(ocr.get("raw_text", {}).get("value", "No text extracted."), language=None)

            # --- Predict button ---
            all_filled = all([scanned_airline, scanned_origin, scanned_destination, scanned_time])

            if not all_filled:
                st.warning("Please ensure **Airline, Origin, Destination, and Time** are filled before predicting.")
            ...
            if st.button(
                ...
                # Parse the time
                time_str = scanned_time.strip()
                try:
                    # Handle various formats: "14:30", "14:30 PM", "1430"
                    time_str_clean = time_str.upper().replace("AM", "").replace("PM", "").strip()
                    if ":" in time_str_clean:
                        hh, mm = map(int, time_str_clean.split(":")[:2])
                    elif len(time_str_clean) == 4 and time_str_clean.isdigit():
                        hh, mm = int(time_str_clean[:2]), int(time_str_clean[2:])
                    else:
                        hh, mm = 12, 0
                    dep_time_int = hh * 100 + mm
                except Exception:
                    dep_time_int = 1200  # Default noon

                # Parse date for month/day/weekday
                from dateutil import parser as date_parser
                try:
                    parsed_date = date_parser.parse(scanned_date, dayfirst=True)
                ...
                dist = distance_lookup.get(f"{scanned_origin}|{scanned_destination}", 1000)

                # Fetch weather
                with st.spinner(f"Fetching weather for {scanned_origin}..."):
                    w_data = weather_service.get_weather(scanned_origin, month=month)

                prediction_input = {
                    'airline': scanned_airline,
                    'origin': scanned_origin,
                    'destination': scanned_destination,
                    'month': month,
                    'day': day,
                    'weekday': weekday,
                ...
                result = predict_flight_delay(prediction_input)

                # Log OCR accuracy for analytics
                st.session_state.ocr_log.append({
                    "timestamp": datetime.now().isoformat(),
                    "overall_confidence": overall,
                    "fields_flagged": sum(
                        1 for f in ["flight_number", "airline", "origin", "destination", "date", "time"]
                        if ocr.get(f, {}).get("low_confidence", True)
                    ),
                })
                ...
                if 'error' in result:
                    ...
                    mc1.metric("Delay Probability", f"{prob}%")
                    mc2.metric("Estimated Delay", format_time(delay))

                    if prob < 20:
                        st.success("**Great News!** Your flight is predicted to be on time.")
                    elif prob < 50:
                        st.warning("**Attention Needed.** Moderate risk of delay. Monitor your airline's live status.")
                    else:
                        st.error("**High Risk Alert!** Significant delays are very likely.")

                    # Weather insight
                    source_label = w_data.get('api_source', 'Live Data')
                    st.caption(
                        f" **Weather Insight ({source_label}):** {w_data['condition']} "
                        f"| Wind: {w_data['wind_speed']} km/h | Vis: {w_data['visibility']} km"
                    )
...
elif st.session_state.page == "scan": render_scan_ticket()
